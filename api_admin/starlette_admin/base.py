...
from starlette_admin.helpers import hash_password
...
from admins import engine


def temp_file():
    with open("temp.py", "w", encoding="utf-8") as f:
        temp_dict = {"ok": "OK"}
        json.dump(temp_dict, f, sort_keys=True, indent=4, ensure_ascii=False)
        headers = {"Content-type": "application/json"}
        try:
            response = requests.get(f"http://{sr}/up/", headers=headers)
...
class BaseAdmin:
    ...
    async def handle_action1(self, request: Request) -> Response:
        pass
    ...
    async def _render_detail(self, request: Request) -> Response:
        ...
        dict_obj = await model.serialize(obj, request, RequestAction.DETAIL)
        from admins.view import AVAILABLE_Menu_dict, AVAILABLE_DB_dict

        if "accessmenu" in dict_obj:
            if isinstance(dict_obj["accessmenu"], list):
                dict_obj.update(
                    {
                        "accessmenu": ", ".join(
                            [
                                AVAILABLE_Menu_dict[i]
                                for i in dict_obj["accessmenu"].split("/")
                            ]
                        )
                    }
                )
                dict_obj.update(
                    {
                        "accessdb": ", ".join(
                            [
                                AVAILABLE_DB_dict[i]
                                for i in dict_obj["accessdb"].split("/")
                            ]
                        )
                    }
                )
    ...
    async def _render_create(self, request: Request) -> Response:
        ...
        try:
            if identity == "adminuserproject_admin":
                dict_obj["password"] = hash_password(dict_obj["password"])
    ...
    async def _render_edit(self, request: Request) -> Response:
        ...
        if identity == "adminuserproject_admin":
            dict_obj.update({"accessmenu": "/".join(dict_obj["accessmenu"])})
            dict_obj.update({"accessdb": "/".join(dict_obj["accessdb"])})
            dict_obj.update({"statususer": "/".join(dict_obj["statususer"])})

        elif identity == "admindbs_admin":

            with Session(engine["DB_admin"]) as session:
                interval_1 = dict_obj["interval_1"]
                interval_2 = dict_obj["interval_2"]
                interval_3 = dict_obj["interval_3"]
                time_1 = dict_obj["time_1"]
                time_2 = dict_obj["time_2"]
                activated_shed = dict_obj["activated_shed"]
                activated_shed_1 = dict_obj["activated_shed_1"]
                activated_dop = dict_obj["activated_dop"]

                stmt_dbs = update(AdminDBs_admin).values(
                ...
                answ_dbs = session.execute(stmt_dbs)
                session.commit()

                stmt_dbs = (
                    update(AdminDBs_admin)
                    .values(
                        activated_shed=activated_shed,
                        activated_shed_1=activated_shed_1,
                        activated_dop=activated_dop,
                    )
                    .where(AdminDBs_admin.Nick == dict_obj["Nick"])
                )

                answ_dbs = session.execute(stmt_dbs)
                session.commit()

                stmt_req_db_status = select(AdminDBs_admin.__table__.columns)
                answ_req_db_status = (
                    session.execute(stmt_req_db_status).mappings().all()
                )

                for i in answ_req_db_status:
                    if (
                        dict_obj["activated_shed"] == True
                        or i["activated_shed"] == True
                    ):
                        activated_shed = True
                        break
                    else:
                        activated_shed = False
                for i in answ_req_db_status:
                    if (
                        dict_obj["activated_shed_1"] == True
                        or i["activated_shed_1"] == True
                    ):
                        activated_shed_1 = True
                        break
                    else:
                        activated_shed_1 = False
                for i in answ_req_db_status:
                    if dict_obj["activated_dop"] == True or i["activated_dop"] == True:
                        activated_dop = True
                        break
                    else:
                        activated_dop = False

                stmt_shed_dbs = update(shed_admin).values(
                    interval_1=interval_1,
                    interval_2=interval_2,
                    interval_3=interval_3,
                    time_1=time_1,
                ...
                answ_shed_dbs = session.execute(stmt_shed_dbs)
                session.commit()
