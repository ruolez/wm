...
print(os.path.dirname(__file__), os.getcwd(), "dirnameeeeeeeeee")
...
from admins.model import AdminUserProject_admin, admin_menu_list, admin_menu
...
class ModelView(BaseModelView):
    ...
    async def find_all(
        ...
        print("find_all_admin-------------------------------<<<<<<<<<<<<<<<<<<")
        request_list = request.path_params["identity"]
        print(
            request.path_params,
            request_list,
            "request_admin___listttttttttttt=--------------------->>>",
        )
        print(order_by, "order_by_adminnnnnnnnnnn")
        session: Union[Session, AsyncSession] = request.state.session
        ...
        print(stmt, type(stmt), "stmt11111_admin-------------------------------->>")

        for field in self.fields:
            if isinstance(field, RelationField):
                stmt = stmt.options(joinedload(getattr(self.model, field.name)))
        print(stmt, type(stmt), "stmt_admin-------------------------------->>")
        print(
            stmt.__dict__, "stmttttttttttt_dict_admin____________________________>>>>>>"
        )

        identity_req = request.path_params["identity"]
        print(identity_req, "identity_reqqqqqqqq============================")
        if identity_req == "adminuserproject_admin":
            with Session(engine["DB_admin"]) as session:

                query_users_menu = select(AdminUserProject_admin.__table__.columns)
                print(engine, "engineeeeeeeeeeee")


                print(query_users_menu, "query_users_menuuuuuuuuuu")
                answ_users_menu = session.execute(query_users_menu).mappings().all()
                print(
                    answ_users_menu,
                    "answ_users_menuuuuu----------------------------",
                )
            from admins.view import (
                AVAILABLE_Menu_dict,
                AVAILABLE_DB_dict,
                AVAILABLE_Menu_dict_rev,
            )


            return answ_users_menu
    ...
    async def create(self, request: Request, data: Dict[str, Any]) -> Any:
        print("create((((((((((((((((((((((((())))))))))))))))))))))))")
        identity_req = request.path_params["identity"]
        print(identity_req, "identity_reqqqqqqqq_create============================")
        print(data, "data1______________________________________")

        if identity_req == "adminuserproject_admin":
            data.update({"accessmenu": "/".join(data["accessmenu"])})
            ...
            with Session(engine["DB_admin"]) as session:
                ...
                print(query_users_menu, "query_users_menuuuuuuuuuu")
                answ_users_menu = session.execute(query_users_menu).mappings().all()
                print(answ_users_menu, "answ_users_menuuuuuuuuuu")
            list_users: list[str] = list()
            if answ_users_menu:
                for i in answ_users_menu:
                    list_users.append(i["username"])

                if created_user in list_users:
                    errors = {
                        "username": f'This username "{created_user}" already exists'
                    }
                    raise FormValidationError(errors)

        print(data, "data2______________________________________")

        if "accessdb" in data:
            if data["accessdb"] and data["accessdb"] != None:
                data.update({"accessdb": "/".join(data["accessdb"])})
        try:
