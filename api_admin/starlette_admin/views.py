...
class BaseModelView(BaseView):
    ...
    async def serialize(
        ...
        if "accessmenu" in obj_serialized:
            if obj_serialized["accessmenu"] and obj_serialized["accessmenu"] != None:
                obj_serialized.update(
                    {
                        "accessmenu": ", ".join(
                            [
                                AVAILABLE_Menu_dict[i]
                                for i in obj_serialized["accessmenu"].split("/")
                            ]
                        )
                    }
                )
        if "accessdb" in obj_serialized:
            if obj_serialized["accessdb"] and obj_serialized["accessdb"] != None:
                print(AVAILABLE_DB_dict, "AVAILABLE_DB_dicttttttttttttttt")
                print(
                    obj_serialized,
                    obj_serialized["accessdb"].split("/"),
                    "obj_serializeddddddddddd_accessdb",
                )
                ...
                obj_serialized.update(
                    {
                        "accessdb": ", ".join(
                            [
                                AVAILABLE_DB_dict[i]
                                for i in obj_serialized["accessdb"].split("/")
                            ]
                        )
                    }
                )

        elif "TypeDB" in obj_serialized:
            obj_serialized.update({"TypeDB": "mssql"})

        print(obj_serialized, "obj_serializedddddddddddddddd_TypeDB")
