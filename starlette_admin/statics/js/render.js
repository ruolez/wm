...
const render = {
  ...
  col_1: function (data, type, full, meta) {
    ...
    if (full._status_quotation == 'DELETED') {
      return ``;


    } else {
      return `
<a href="${full._quotation_update}?QuotationNumber=${full.QuotationNumber}&InvoiceNumber=${full.InvoiceNumber}&Status=${full.Status}">
...
<a href="${full._quotation_return}?QuotationNumber=${full.QuotationNumber}&InvoiceNumber=${full.InvoiceNumber}&Status=${full.Status}">
