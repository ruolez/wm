...
const render = {
  ...
  col_1: function (data, type, full, meta) {
          ...
          return `
<a href="${full._update_quotation_details}?DBname=${ full._DBname }&LineID=${ full.LineID }
&Qty=${ full.Qty }&UnitPrice=${ full.UnitPrice }&QuotationID=${ full._QuotationID }&status=view_details">
...
<a href="${full._delete_quotation_details}?DBname=${ full._DBname }&LineID=${ full.LineID }
&ProductDescription=${ full.ProductDescription }&QuotationID=${ full._QuotationID }&status=del_details">
  ...
  text: function render(data, type, full, meta, fieldOptions) {
    ...
    <span class="align-middle d-inline-block text-truncate" data-toggle="tooltip" data-placement="bottom" 
    title='${data}' style="max-width: 30em;">${data}</span>
