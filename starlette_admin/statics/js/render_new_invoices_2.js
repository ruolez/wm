function escape(value) {
  ...
  const render = {
    ...
    col_3: function (data, type, full, meta) {
      let DBname = $('#DB').text();
      
      let printPDFButton = `<button type="button" class="btn btn-outline-primary btn-inv" style="font-size: 10px;" id="PrintPdfButton" onclick="PrintInvoice({ DBname: '${DBname}', InvoiceID: '${full.InvoiceID}', InvoiceNumber: '${full.InvoiceNumber}', just_download: false})">
         Print PDF
      </button>`;
      let seeDetailsButton = `<a href="/project/InvoicesDetails_tbl?DBname=${DBname}&InvoiceID=${full.InvoiceID}&InvoiceNumber=${full.InvoiceNumber}" onclick="setSessionStorage()">
        <button type="button" class="btn btn-primary btn-inv" style="font-size: 10px;">See Details</button>
        </a>`;
      let sendEmailButton = `
        <button type="button" class="btn btn-outline-primary btn-inv" style="font-size: 10px;" 
                onclick="SendEmail({ DBname: '${DBname}', InvoiceID: '${full.InvoiceID}', InvoiceNumber: '${full.InvoiceNumber}', AccountNo: '${full.AccountNo}'})">
          Send Email
        </button>
      `;
      return `
