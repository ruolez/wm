    ...
    var table = new DataTable("#browse-items-table", {
      ...
      ajax: function (data, callback, settings) {
        ...
        if (inputValue?.length > 2) {
          ...
          var $emptyElement = $("#browse-items-table .dataTables_empty");
...
function autocomplete(inp) {
  ...
  inp.addEventListener("input", Debounce(async function (e) {
    ...
    if (!$('#bnameInput').val()|| !$('#acnumInput').val()  ){
      DisableTab();
      DisableSearch();
    }
...
function FillShipVia(shipviaid) {
  if (shipviaid){
    ...
    if ($dropdownItem.length > 0) {
      let text = $dropdownItem.text();
      $("#shipvDropdown").text(text);
      $("#shipvDropdown").attr("data-value", shipviaid);
    }
  }
}
...
function PrepareMainFields() {
  return {
    ...
    shipvia: parseInt($("#shipvDropdown").attr("data-value") || 0),
...
if (qdate) {
  ...
  FillTerms(data?.terms||0);
  FillSalesRep(data?.srep||0);
  FillShipVia(data?.shipvia||0);
...
$(document).ready(function () {
  // Назначаем событие клика только на <a> внутри <aside> и используем .one() для однократного выполнения
  $('aside').one('click', 'a', function (e) {
    e.preventDefault(); // Предотвращаем переход по ссылке
    ShowConfirmModal(this.href); // Открываем модальное окно с нужной ссылкой
  });
});
  ...
  data.forEach(function(row, index) {
    ...
    if (priceLookup[upc] !== undefined) {
      ...
      if (unitPriceCell.length > 0) {
        ...
        unitPriceCell.add('black-text-color');
      }
    }
  });
...
// Преобразуем серверное время в объект даты с учетом временной зоны 'America/Chicago'
let chicagoDate = new Date(
  new Intl.DateTimeFormat('en-US', {
    timeZone: 'America/Chicago',
    hour12: true,
    year: 'numeric', month: '2-digit', day: '2-digit',
    hour: '2-digit', minute: '2-digit'
  }).format(new Date(readOnlyTime))
);

// Получаем текущее время
let now = new Date();

// Копируем текущее время для теста и добавляем 2 часа СДВИГА ТОЛЬКО ДЛЯ ПРОВЕРКИ
let testNow = new Date(now);
// // Сдвиг на 2 часа вперед

// Считаем разницу между ОРИГИНАЛЬНЫМ текущим временем и временем с сервера
let diffMs = now.getTime() - chicagoDate.getTime();

// Переводим разницу в часы и минуты
let diffTotalMinutes = Math.floor(diffMs / (1000 * 60)); // Общие минуты
let diffHours = Math.floor(diffTotalMinutes / 60); // Часы
let diffMinutes = diffTotalMinutes % 60; // Оставшиеся минуты

// Форматируем результат
let formattedTime = `${diffHours} h ${diffMinutes.toString().padStart(2, '0')} min`;

// Генерируем контент для popover
let popoverContent = `<strong>User:</strong> ${readOnlyUser}<br><strong>Elapsed Time:</strong> ${formattedTime}`;

// Устанавливаем контент для popover и инициализируем его
...
async function ShowConfirmModal(href=null){
  ...
  if (!IsChanged()){
   await ProcessUnlock();
    ...
    if (status === 'locked' ||status === 'converted'|| readonly==1) {
      ...
      $('#noConfirmCloseButton').off('click').on('click',async function(e) {
        await ProcessUnlock();
        CloseQuatation(href);
    });
...
$(document).ready( function () {
  ...
  $('#readonlyCheck').on('change',async function () {
    if (!$(this).is(':checked')) {
      // Checkbox is unchecked (read-only disabled)
     await ProcessUnlock(this);
    } else {
...
async function ProcessUnlock(object=null){
...
if (qid){
  ...
  try {
    const response = await UnlockQuotation(obj);
    if(response?.error === undefined){
      toastr.error(`Where is error??`);
    }

    if (response?.error == 0 && object) {
    ...
    toastr.error(`Unlock failed: ${error||'No error recived'}`);
...
function saveInputToCookie($input) {
  setCookie('QuotationID', $input.val());
...
$(document).ready(function () {
  ...
  let $input = $('#QuotationID');


  const observer = new MutationObserver(() => saveInputToCookie($input));

  observer.observe($input[0], { attributes: true, attributeFilter: ['value'] });


  $input.on('change', saveInputToCookie);
...
$(document).ready(function () {
  $('a[data-bs-toggle="tab"]').on('shown.bs.tab', function (e) {
    ...
    const $cusmordetails = $('#cusmordetails');
    if (activeTabId === '#tabs-details-ex3') {
      $cusmordetails.hide();
      LoadQuotationDetails();

    } else {
      $cusmordetails.show();
      CleanDatable("#details-table"); // Очистить таблицу
      RemoveKeyUpDownListeners('details-table'); // Удалить слушатели событий
    }
  });
  ...
  async function LoadQuotationDetails() {
    let Data = {
      Db: GetDb(),
      AccountNumber: $('#acnumInput').val(),
      Items:JSON.stringify(GetItemsUPC())
    };
    $('#spinner-container').css({"display": "flex"});
    $('#details-table-container').css({"display": "none"})
    await GetQoutationDetails(Data).then(async function(response) {

      await initDetailsDataTable(response);
...
function GetItemsUPC() {
  let dataArray = $('#items-table').DataTable().data().toArray();

  let result = dataArray.map(row => ({
    "UPC": row.UPC
  }));
...
async function initDetailsDataTable(response){
  CleanDatable("#details-table");

  let table = new DataTable("#details-table", {
    dom: 'rtS',
    scrollY: true,
    scrollX: true,
    paging: false,
    ...
    select: true,
    ajax: function (data, callback, settings) {
      $('#details-table-container').css({"display": "block"});
    ...
    initComplete: function () {
        ...
        // Добавляем строку фильтров под заголовком

        // Для каждой колонки добавляем фильтр (input или select)
        this.api().rows().every(function (index) {
          let table = $('#details-table').DataTable();
          let rowData = table.row(index).data(); // Получаем данные строки
          $(table.cell(index, 9).node())
                   .css('background-color', 'antiquewhite')
                   .addClass('hilight');
        // Проверяем условие для LastPriceLevel в колонке номер 1
      //   if (rowData.LastPriceLevel === 'Standard') {
      //     $(table.cell(index, 9).node())
      //         .css('background-color', 'antiquewhite')
      //         .addClass('hilight');
      //  } else if (rowData.LastPriceLevel == 'DeliveryB') {
      //     $(table.cell(index, 3).node())
      //         .css('background-color', 'antiquewhite')
      //         .addClass('hilight');
      //  } else if (rowData.LastPriceLevel == 'Custom') {
      //     $(table.cell(index, 4).node())
      //         .css('background-color', 'antiquewhite')
      //         .addClass('hilight');
      //  }

        })
        AddKeyUpDownListeners('details-table');
  ...
  function DisableTab(){
    $('a[href="#tabs-details-ex3"]').addClass('disabled');
  }

  function EnableTab(){
    $('a[href="#tabs-details-ex3"]').removeClass('disabled');
  }